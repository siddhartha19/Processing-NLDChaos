/* Here you can create any kind of flow field in Cartesian coordinates.
Use the variables "irel" to represent x and "jrel" to represent y. 

The functions below return dudt and dvdt, of your 2D Dynamical System.
*/
...
void showVecField() {
  ...
  for (float i = 0; i <= width; i += step) {
    for (float j = 0; j <= height; j += step) {
      float irel = map(i, 0, width, -range, range);
      float jrel = map(j, 0, height, range, -range); //-- Again flipping y to set y=0 to the bottom of the screen i.e. to be mapped to -range
